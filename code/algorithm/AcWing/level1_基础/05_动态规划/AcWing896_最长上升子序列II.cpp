...
// 如果当前位置我更大, 就把我的替换上, 如果后面碰到大的就把大的插我后面
int main()
{
    cin >> n;
    ...
    int len = 0;
    q[0] = -2e9;    // 处理边界, 为了保证 q[] 上小于 a[i] 的某个数一定存在保证边界, 将 q[0] 赋值为哨兵

    // 枚举所有数
    for (int i = 0; i < n; ++ i ) 
    {
        int l = 0, r = len; 
        while (l < r)                           // 在 q[] 上二分出小于 a[i] 最大的数, 再把 a[i] 更新在该长度后面
        {
            int m = (l + r + 1) >> 1;
            // 如果 q[m] < a[i] 那么, q[m] 一定在 a[i] 的左边
            if (q[m] < a[i]) l = m; 
            else r = m - 1;
        }
