...
//        (000~999 有 1000 种数字, 第 4 位 1 保持不变, abc 每改变自己一位就能得到 1000 个数)
//     2) xxx = abc
//        (这里的 d 指的的上限)
//         - 2.1) d < 1, abc1yyy > abc0def, 0 (第 4 位小于 1, 那么怎么取都取不到)
//         - 2.2) d = 1, yyy = 000~efg (abc 已经到达上限了, 后三位就不能超过上限), efg+1, abc1yyy ~ abc1efg
//         - 2.3) d > 1, yyy = 000~999, abc1yyy~abcdefg(d > 1), 那么第 4 位的 1 有 1000 个, 只要 yyy 不进位 
// 
// 注意此处的 d 指的是 abc'd'efg 的 d, 而非 xxx1yyy 的 1
// 这些情况代表的是一个区间可以取到的数
