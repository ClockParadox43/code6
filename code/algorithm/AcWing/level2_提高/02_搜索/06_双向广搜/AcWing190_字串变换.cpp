// 该题也是求某个状态变换成另外一个状态最少多少步数
// 因为可变换的状态比较多, 扩展出的可能性也比较多, 所以用双向广搜
// ps: 搜索宽度太宽, 适合用双向
//
// 实现1: 从两个方向分别扩展一步
// 实现2: 每次选择当前队列元素数量较少的队列进行扩展
// 
// ps:双向广搜的时候要扩展一整层的点, 否则答案可能有错, a + b + 2
//    但是如果a扩展完一层都没有搜到队列b搜到的点, 那答案不存在当前前这一层, 只能搜下一层 

#include <iostream>
#include <cstring>
#include <algorithm>
#include <queue>
#include <unordered_map>    // 判断哪些状态已经搜过了
    ...
    while (q.size() && da[q.front()] == d)  
    {
        ...
        for (int i = 0; i < n; ++ i )
            for (int j = 0; j < t.size(); ++ j )
                if (t.substr(j, a[i].size()) == a[i])   // 判断当前子串(j开始,规则长度的子串)和规则是否相等: sub t 这个状态, 判断经过截取每一位是否可以和规则 a[i] 相等 
                {
                    // 如果相等, 就把a[i]替换成b[i]
