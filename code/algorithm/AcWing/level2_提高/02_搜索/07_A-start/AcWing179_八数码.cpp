...
//             4)挑一个最小的点来扩展
//             5)当终点第一次出队的时候 break
// ps: A* 算法和 Dijsktra 很像, A* 算法可以处理的边权不一定是为 1, 只要没负权即可
//     A* 一定要是有解的情景才能使用, 因为就算没解, A* 也会将所有状态都搜索一遍
//    这种情况 A* 算法效率低于普通 DFS
...
// 反证法:
//     假设终点 t 出队时不是最小值, 既然不是最小值, 那么 t 一定严格大于最优
//     队列当中此时也存在最优解
//     因为 f(u) 小于真实值, 所以 d(u) + f(u) ≤ d(u) + g(u) = 最优
//     因为 t 不是最优解, 假设当前距离为dis 所以 dis > 最优d ≥ d(u) + f(u)
//     因为优先级队列中只会出最小值, 现在出现了 < t的值, 所以该情况就矛盾了
